...
import { signUp, useGoogleSignIn } from '../../utils/auth';
...
const SignupScreen = () => {
  ...
  const { promptAsync } = useGoogleSignIn(); // ✅ Google Sign-In Hook
                ...
                if (error) {
                  ...
                  try {
                    ...
                    if (userSnap.exists()) {
                      ...
                      Alert.alert('Welcome!', `Account created for ${userData.firstName}`);
                      navigation.reset({
                        index: 0,
                        routes: [{ name: 'MainTabs' }],
                      });                      
                    } else {
                      Alert.alert('Error', 'User data not found after creation.');
                    }
                  } catch (firestoreError) {
                    console.error("Firestore Error:", firestoreError);
                    Alert.alert('Error', 'Failed to save or fetch user info.');
                  }
                }
              }}
